...
void* raytrace(void* thrd) {
  ...
  /* for every pixel */
  for( int px=thread; px<width; px+=numThreads )
    {
...
{
    numThreads = argc == 2 ? atoi( argv[1] ) : 0;
    ...
    for (int i = 0; i < numThreads; i++) {
      nums[i] = i;
      int ret = pthread_create(&tids[i], NULL, raytrace, &nums[i]);
      if (ret) {
	exit(1);
      }
    }
    ...
    for (int x = 0; x < numThreads; x++) {
      int ret = pthread_join(tids[x], NULL);
      if (ret) {
	exit(1);
      }
    }
	 ...
	 */
	printf( "%.0f %.0f %.0f\n",
		scaled_color[a][b][0], scaled_color[a][b][1], scaled_color[a][b][2] );
      }
      printf( "\n" );
    }
